...
def excel_generate(date_eg):
    ...
    xls = RemoteExcel(r'D:\\60_openadoor\BlingBlingMoney\\模板.xlsx')
...
def www2pic(fromurl, savepath):
    ...
    datestrxls = time.strftime('%Y-%m-%d', time.localtime(time.time()))
    savepath = r"D:\\60_openadoor\BlingBlingMoney\\" + savepath + datestrxls + '.png'
...
def sina2pic(date_sina):
    ...
    savepath = r"D:\\60_openadoor\BlingBlingMoney\\hqsh000001" + date_sina + '.png'
    driver.save_screenshot(savepath)
    im = Image.open(savepath)
...
def acq_pic_path_jrj(date_wwwths_ztfp, savepath_wwwths_ztfp):
...
def to_html(date_html, save_path, list_path_pic_to_html):
    ...
    pic_ztfp = list_path_pic_to_html[6]
    text_ztfp = list_path_pic_to_html[7]
    text_ztfp_concolusion = list_path_pic_to_html[8]
    f = open(save_path, 'w')
...
if __name__ == '__main__':
    ...
    pic_wxgzh_path = pic_base_path + 'wxgzh_pic' + date_str_today + '.png'
    ...
    list_pic_path = [iciba(date_str_today, pic_words_everyday_iciba),  # 0.每日一图
                    ...
                    # data2pic_ths_ztfp_sp(date_str_today, '', acq_pic_path_jrj(date_str_today, pic2text_json_out_path), zt_detail_today(date_str_today), zt_detail_yestoday()) ] # 6.ztfp
                    ztfp[0],
                    ztfp[1],
                    ztfp[2], ] # 6.ztfp
